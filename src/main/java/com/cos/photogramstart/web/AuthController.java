...
public class AuthController {
	 ...
	 * authService; } // @Controller를 사용하면 스프링에서 컨테이너를 관리하는 메모리로 AuthController 로드를
	 * 하여 메모리 객체를 생성하여 관리를 하게 되는데 객체를 생성하기위해서는 첫번째 조건, 생성자를 만들어 실행을 시켜야 한다. 지금 생성된
	 * AuthController를 사용할 경우 IoC등록된 애들중에서 authService에 해당하는 Type이 있으면
	 * this.authService 넣어준다(의존성 주입) 없으면 오류가 발생. 현재 오류가 발생하지 않는 이유는
	 * AuthService에 @Service를 어노테이션 해주어 Ioc 등록시켜 주었기 때문이다.
	 */
